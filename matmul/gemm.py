...
def _tile_workspace_bytes_per_elem(cfg: Config) -> int:
    ...
    Four T×T tiles are live at the peak of each k-step: the two operand tiles
    (``a_dev``, ``b_dev``), the accumulator (``acc``), AND the product returned
    by ``backend.matmul`` -- ``torch.bmm`` cannot write into either operand, so
    its output is a distinct allocation that co-exists with ``acc`` during the
    ``acc += prod`` add. The product is produced in the operand/compute dtype,
    so it costs one more ``_tile_operand_bytes`` tile. Omitting it under-counts
    the working set by ~4/3 and makes ``auto_tile`` over-size T (see #95).
    """
...
def auto_tile(n: int, cfg: Config, backend: Backend) -> int:
    ...
    Working set per (i,j,k) step on the device:
        acc (T x T, acc_dtype)  +  (A-tile + B-tile + matmul output)
        (T x T, operand bytes each)
    """
