...
def _fill_iota(mat: np.ndarray, seed: int = 0) -> None:
    ...
    ``seed`` shifts the pattern so distinct seeds produce distinct matrices, like
    ``_fill_random``/``_fill_lowrank``. Without it every ``generate`` call returns
    the same array, so a couple's A and B (and every pair) come out identical
    whenever ``fill='iota'``."""
    n = mat.shape[0]
    block = max(1, min(n, (256 * 1024**2) // (n * 8)))
    cols = np.arange(n)
