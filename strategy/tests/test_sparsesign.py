"""CPU-safe tests for the ``sparsesign`` (OSNAP-style) sketch transform.

No GPU required — uses the smoke-test CPUBackend so these always run in the
contributor CI path (`pytest strategy/tests`). Mirrors ``test_nystrom.py`` for
the shared Transform contract, plus a reproducible check that signed mixing
beats plain single-column landmarks on decaying-spectrum data.
"""
...
def test_sparsesign_rejects_bad_m():
    ...
    for bad_m in (0, -1, n + 1):
        try:
            SparseSignSketchTransform(seed=0).basis(n, bad_m, backend, np.float32, A=A, B=B)
            assert False, f"expected ValueError for m={bad_m}"
        except ValueError as e:
            assert "sparsesign" in str(e)


def test_sparsesign_basis_shape_orthonormal():
    backend = CPUBackend(verbose=False)
    n, m = 64, 16
    ...
    Q = SparseSignSketchTransform(seed=0).basis(n, m, backend, np.float32, A=A, B=B)
...
def test_sparsesign_seed_deterministic():
    backend = CPUBackend(verbose=False)
    n, m = 48, 12
    A = generate(n, np.float32, False, None, seed=3, fill="random")
    B = generate(n, np.float32, False, None, seed=4, fill="random")
    Q1 = backend.to_host(SparseSignSketchTransform(seed=7).basis(n, m, backend, np.float32, A, B))
    Q2 = backend.to_host(SparseSignSketchTransform(seed=7).basis(n, m, backend, np.float32, A, B))
    Q3 = backend.to_host(SparseSignSketchTransform(seed=8).basis(n, m, backend, np.float32, A, B))
    assert np.allclose(Q1, Q2)
    assert not np.allclose(Q1, Q3)


def test_sparsesign_recovers_low_rank_product():
    # M comfortably exceeds 3x the rank: any reasonable 3-way basis should
    # recover the product almost exactly.
    backend = CPUBackend(verbose=False)
    n, rank, m = 128, 4, 60
    ...
    Q = backend.to_host(SparseSignSketchTransform(seed=0).basis(n, m, backend, np.float64, A, B))
    P = Q @ Q.T
    C = A @ B
    Chat = P @ A @ P @ B @ P
...
def test_sparsesign_basis_flops_between_nystrom_and_rsvd():
    n, m = 12000, 1500
    ss = SparseSignSketchTransform().basis_flops(n, m)
    ny = NystromTransform().basis_flops(n, m)
    rs = RandomizedSVDTransform().basis_flops(n, m)
    s = SparseSignSketchTransform.sparsity
    expected = 2.0 * n * m * m + float(s - 1) * n * m
    assert ss == expected
    assert ss > ny > 0.0
    assert ss < rs
...
def test_sparsesign_beats_nystrom_on_decaying_spectrum():
    """Signed mixing should recover the product more accurately than plain
    single-column landmarks on decaying-spectrum fills, at the same (host
    gather, no-GEMM) cost class. Mean over several seeds, two regimes."""
    backend = CPUBackend(verbose=False)
    for n, rank, m, trials in ((256, 64, 64, 6), (256, 32, 64, 6)):
        ...
        for t in range(trials):
            ...
            Qss = backend.to_host(
                SparseSignSketchTransform(seed=seed).basis(n, m, backend, np.float64, A, B)
            )
            ny_accs.append(1.0 - _rel_err(A, B, Qny))
            ss_accs.append(1.0 - _rel_err(A, B, Qss))
        assert np.mean(ss_accs) > np.mean(ny_accs), (
            f"n={n} rank={rank} m={m}: sparsesign mean acc {np.mean(ss_accs):.4f} "
